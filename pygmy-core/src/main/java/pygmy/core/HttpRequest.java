package pygmy.core;

import lombok.extern.slf4j.Slf4j;

import java.io.*;
import java.net.HttpURLConnection;
import java.net.Socket;
import java.net.URLDecoder;
import java.util.*;


/**
 * This holds the http request data a given request.
 */
@Slf4j
public class HttpRequest extends Request {

    private static int nextRequestId = 0;

    private UrlMatch matchingRule;

    private Integer requestId = null;

    private String scheme;

    private String method;

    private String url;

    private String query;

    private String protocol;

    private int major;

    private int minor;

    private HttpHeaders headers;

    private InternetInputStream bodyStream;

    private Map<String, String> httpVariableMap;

    private long timeStamp;

    private Map<String, MultipartUpload> uploads = Collections.emptyMap();

    public HttpRequest(String aScheme, Socket aConnection, Properties serverConfig) {
        super(aConnection, serverConfig);
        scheme = aScheme;
        connection = aConnection;
        init();
    }

    public HttpRequest(String url, Properties serverConfig, boolean isInternal) {
        super(null, serverConfig);
        init();
        method = "GET";
        scheme = "http";
        parseUrl(url);
        protocol = "HTTP/1.0";
        major = 1;
        minor = 0;
        headers = new HttpHeaders();
        setIsInternal(isInternal);
    }

    public boolean readRequest(InputStream aStream) throws IOException {
        BufferedLimitedInputStream bufferedLimited = new BufferedLimitedInputStream(aStream);
        InternetInputStream stream = new InternetInputStream(bufferedLimited);
        String startLine = null;
        try {
            startLine = readHttpCommand(stream);
            if (startLine == null) {
                return false;
            }
            if (protocol.equals("HTTP/1.0")) {
                major = 1;
                minor = 0;
            } else if (protocol.equals("HTTP/1.1")) {
                major = 1;
                minor = 1;
            } else {
                throw new HttpProtocolException(HttpURLConnection.HTTP_VERSION, "Protocol " + protocol + " not supported.");
            }

            headers = new HttpHeaders(stream);
            bufferedLimited.limit(headers.getContentLength() != null ? headers.getContentLength() : 0);
            bodyStream = stream;
            createVariableMap();
        } catch (NoSuchElementException e) {
            throw new HttpProtocolException(HttpURLConnection.HTTP_NOT_FOUND, "Bad request " + startLine);
        } catch (NumberFormatException e) {
            throw new HttpProtocolException(HttpURLConnection.HTTP_LENGTH_REQUIRED, "Content Length was not a number or not supplied.");
        }
        return true;
    }

    private void createVariableMap() throws IOException {
        httpVariableMap = createQueryMap(query);
        String contentType = headers.getContentType();
        if ("application/x-www-form-urlencoded".equals(contentType)) {
            httpVariableMap.putAll(createQueryMap(readPostAsString()));
        } else if ("multipart/form-data".equals(contentType)) {
            parseMultipart();
        }
    }

    private void parseMultipart() throws IOException {
        uploads = new HashMap<String, MultipartUpload>();
        Map<String, String> options = headers.getHeaderOptions("Content-Type");
        String boundary = "--" + options.get("boundary");
        String line;
        while ((line = bodyStream.readline()) != null) {
            if (line.equals(boundary)) {
                Map<String, String> header = bodyStream.readHeader();
                Map<String, String> contentDisposition = HttpHeaders.getHeaderOptions(header, "Content-Disposition");
                if (contentDisposition.containsKey("filename")) {
                    MultipartUpload multipart = new MultipartUpload(this, bodyStream, header, boundary, headers.getCharset());
                    uploads.put(multipart.getName(), multipart);
                } else {
                    // todo read post section until boundary condition.  Need to check for end boundary
                }
            } else if (line.startsWith(boundary) && line.endsWith("--")) {
                // we're done.
                break;
            }
        }
    }

    private void init() {
        method = null;
        url = null;
        query = null;
        protocol = null;
        httpVariableMap = null;
        timeStamp = System.currentTimeMillis();
        requestId = nextRequestId++;
    }

    public Integer getRequestId() {
        return requestId;
    }

    private String readHttpCommand(InternetInputStream stream) throws IOException {
        String startLine = null;
        do {
            startLine = stream.readline();
            if (startLine == null) return null;
        } while (startLine.trim().length() == 0);

        StringTokenizer tokenizer = new StringTokenizer(startLine);
        method = tokenizer.nextToken();
        parseUrl(tokenizer.nextToken());
        protocol = tokenizer.nextToken();

        return startLine;
    }

    private void parseUrl(String aUrl) {
        int queryIndex = aUrl.indexOf('?');
        if (queryIndex < 0) {
            url = aUrl;
        } else {
            url = aUrl.substring(0, queryIndex);
            if ((queryIndex + 1) < aUrl.length())
                query = aUrl.substring(queryIndex + 1);
        }
    }

    public String getRequestHeader(String key) {
        return headers.get(key);
    }

    public String getRequestHeader(String key, String defaultValue) {
        String val = getRequestHeader(key);
        return (val == null) ? defaultValue : val;
    }

    public String getMethod() {
        return method;
    }

    public String getUrl() {
        return url;
    }

    public String getQuery() {
        return query;
    }

    public String getParameter(String key) {
        return httpVariableMap.get(key);
    }

    public MultipartUpload getUpload(String key) {
        return uploads.get(key);
    }

    private String readPostAsString() throws IOException {
        byte[] postData = new byte[headers.getContentLength()];
        int length = -1;
        int offset = bodyStream.read(postData);
        while (offset >= 0 && offset < postData.length) {
            length = bodyStream.read(postData, offset, postData.length - offset);
            if (length == -1) {
                break;
            }
            offset += length;
        }
        return new String(postData, headers.getCharset());
    }

    public Set<String> getParameterNames() {
        return httpVariableMap.keySet();
    }

    private Map<String, String> createQueryMap(String query) {
        Map<String, String> queryMap = new TreeMap<String, String>();
        if (query == null) {
            return queryMap;
        }

        query = query.replace('+', ' ');
        StringTokenizer st = new StringTokenizer(query, "&");
        try {
            while (st.hasMoreTokens()) {
                String field = st.nextToken();
                int index = field.indexOf('=');
                if (index < 0) {
                    queryMap.put(URLDecoder.decode(field, "UTF-8"), "");
                } else {
                    queryMap.put(URLDecoder.decode(field.substring(0, index), "UTF-8"),
                            URLDecoder.decode(field.substring(index + 1), "UTF-8"));
                }
            }
        } catch (UnsupportedEncodingException e) {
        }

        return queryMap;
    }

    public String getScheme() {
        return scheme;
    }

    public String getProtocol() {
        return protocol;
    }

    public boolean isKeepAlive() {
        return headers.isKeepAlive() && major >= 1 && minor > 0;
    }

    public int getMajorVersion() {
        return major;
    }

    public int getMinorVersion() {
        return minor;
    }

    public long getTimestamp() {
        return timeStamp;
    }

    public String getProperty(String key) {
        return getProperty(key, null);
    }

    public HttpHeaders getHeaders() {
        return headers;
    }

    public String toString() {
        return method + " " + url + ((query != null) ? "?" + query : "") + " " + protocol;
    }

    public String serverUrl() {
        return getProperty("url");
    }

    public String createUrl(String absolutePath) throws IOException {
        return absolutePath;
    }

    public boolean isProtocolVersionLessThan(int aMajor, int aMinor) {
        return (major <= aMajor && minor < aMinor);
    }

    public UrlMatch getUrlMatch() {
        return matchingRule;
    }

    public void setUrlMatch(UrlMatch match) {
        this.matchingRule = match;
        httpVariableMap.putAll(matchingRule.getValues());
    }

    public String getContentType() {
        return headers.getContentType();
    }

    public Integer getContentLength() {
        return headers.getContentLength();
    }

    public String getCharacterEncoding() {
        return headers.getCharset();
    }

    public InputStream getInputStream() {
        return bodyStream;
    }

    public Reader getReader() throws UnsupportedEncodingException {
        return new BufferedReader(new InputStreamReader(bodyStream, getCharacterEncoding()));
    }

    protected File getTemporaryStorage() {
        return new File(System.getProperty("java.io.tmpdir"));
    }

    public void cleanup() {
        for (MultipartUpload upload : uploads.values()) {
            if (upload.getUpload().exists()) {
                if (!upload.getUpload().delete()) {
                    log.warn("Upload [" + upload.getUpload().getAbsolutePath() + "]:(" + upload.getUpload().length() + ") could not be deleted!");
                }
            }
        }
    }

    public String getAcceptedEncoding() {
        String encoding = "identity";
        float max = 0.0f;

        List<Map<String, String>> acceptEncodings = headers.getHeaderListOptions("Accept-Encoding");
        for (Map<String, String> encodings : acceptEncodings) {
            String e = encodings.get("Accept-Encoding");
            float q = 1.0f;
            if (encodings.containsKey("q")) {
                q = Float.parseFloat(encodings.get("q"));
            }
            if (q > max && q > 0) {
                encoding = e;
                max = q;
            }
        }

        return encoding;
    }
}
